"""
Generate multi-view videos from a single input image using cosmos_predict1 environment.

This module uses the cosmos_predict1 environment (via GEN3C/Cosmos) to generate 
multi-view videos from a single input image. The generation process requires 
running twice - once for a left rotation and once for a right rotation.

The trajectory for video generation can be configured in the examples/configs 
files by modifying the gen3c_config.trajectory parameter:
- "spin_left": Generate video with left rotation trajectory
- "spin_right": Generate video with right rotation trajectory

Output:
    Running this script generates the following files under 
    3d_result/{scene_name}/stage1_reconstruction/:
    - {scene_name}_spin_left_images/: Directory containing left rotation frame images
    - {scene_name}_spin_right_images/: Directory containing right rotation frame images
    - {scene_name}_spin_left.mp4: Video file with left rotation trajectory
    - {scene_name}_spin_right.mp4: Video file with right rotation trajectory
"""
...
# Add submodules/GEN3C to Python path to find cosmos_predict1 module
gen3c_path = os.path.join(os.path.dirname(__file__), "../../submodules", "GEN3C")
if os.path.exists(gen3c_path):
    sys.path.insert(0, gen3c_path)
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--config_path",
        type=str,
        default="examples/configs/Monet_venice.yaml",
    ...
    for i, input_dict in enumerate(prompts):
        ...
        try:
            generated_w2cs, generated_intrinsics = generate_camera_trajectory(
                ...
                center_depth_y=args.center_depth_y,
                device=device.type,
                radius=args.radius,
        ...
        if args.save_buffer:
            ...
            # save rendered_warp_images to video
            # Convert tensor to numpy and save as images
            warp_save_dir = os.path.join(
                args.video_save_folder,
                f"{'batch_' + str(i) if args.batch_input_path else args.video_save_name}_warp_images"
            )
            os.makedirs(warp_save_dir, exist_ok=True)
            
            # rendered_warp_images shape: [B, T, n_i, C, H, W]
            warp_images = rendered_warp_images.squeeze(0)  # Remove batch dim
            
            all_warps_video = []
            
            for t in range(warp_images.shape[0]):  # For each time step
                frame_warps = []
                for n in range(warp_images.shape[1]):  # For each warp version
                    img = warp_images[t, n].cpu()  # [C, H, W]
                    img = ((img * 0.5 + 0.5) * 255).numpy().transpose(1, 2, 0).astype(np.uint8)
                    frame_warps.append(img)
                
                if frame_warps:
                    combined_frame = np.concatenate(frame_warps, axis=1)
                    all_warps_video.append(combined_frame)
            
            if all_warps_video:
                warp_video_path = os.path.join(
                    args.video_save_folder,
                    f"{'batch_' + str(i) if args.batch_input_path else args.video_save_name}_warps.mp4"
                )
                save_video(
                    video=np.array(all_warps_video),
                    fps=args.fps,
                    H=all_warps_video[0].shape[0],
                    W=all_warps_video[0].shape[1],
                    video_save_quality=5,
                    video_save_path=warp_video_path,
                )
                log.info(f"Saved warp video to {warp_video_path}")
        ...
        for num_iter in range(1, num_ar_iterations):
            ...
            # print(f"Exporting to colmap {args.video_save_folder}")
            # cache.export_to_colmap(args.video_save_folder)
            # exit()
        ...
        images_save_dir = os.path.join(
            args.video_save_folder,
            args.video_save_name+"_images"
        )
        os.makedirs(images_save_dir, exist_ok=True)

        os.makedirs(os.path.dirname(video_save_path), exist_ok=True)
        for i, frame in enumerate(final_video_to_save):
            image_save_path = os.path.join(images_save_dir, f"{i}.png")
            cv2.imwrite(image_save_path, cv2.cvtColor(frame, cv2.COLOR_RGB2BGR))
...
if __name__ == "__main__":
    ...
    if args.config_path is not None:
        ...
        args.video_save_name = config["scene_name"]+"_"+config["gen3c_config"]["trajectory"]
        args.video_save_folder = config["work_dir"] + "/" + "stage1_reconstruction"
        os.makedirs(args.video_save_folder, exist_ok=True)
        args.center_depth = config["gen3c_config"]["center_depth"]
        args.center_depth_y = config["gen3c_config"]["center_depth_y"]
        args.radius = config["gen3c_config"]["radius"]
        args.guidance = 1
        args.input_image_path = config["data_path"] +"/" + config["scene_name"] + ".png"
        args.trajectory = config["gen3c_config"]["trajectory"]
        args.arc_scale = config["gen3c_config"]["arc_scale"]
        args.seed = config["seed"]

        # For efficient GEN3C inference, you can refer to https://github.com/nv-tlabs/GEN3C
        args.foreground_masking = True
        ...
        args.disable_guardrail = True
        args.disable_prompt_encoder = True
    demo(args)
