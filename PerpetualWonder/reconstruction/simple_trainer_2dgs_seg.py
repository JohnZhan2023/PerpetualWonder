"""
2D Gaussian Splatting trainer with segmentation support.

This module reconstructs a Gaussian scene from camera poses and initial point clouds 
solved by COLMAP. It utilizes masks generated by SAM (Segment Anything Model) to 
separate foreground and background during the reconstruction process.

Output:
    Running this script generates the following directory under 
    3d_result/{scene_name}/stage1_reconstruction/:
    - 3d/ckpts/: Directory containing checkpoint files for the trained Gaussian splatting model
"""
...
class Config:
    ...
    depth_lambda: float = 2e-3
    depth_end_iter: int = 6000
    ...
    # Enable sky gaussians initialization
    sky_gaussians: bool = False
    # Number of sky gaussians to initialize
    sky_num_pts: int = 100
...
def create_splats_with_optimizers(
    ...
    if sky_gaussians:
        print("Initializing sky gaussians...")
        camera_centers = []
        for camtoworld in parser.camtoworlds:
            camera_center = camtoworld[:3, 3]
            camera_centers.append(camera_center)
        camera_centers = np.array(camera_centers)
        avg_camera_center = np.mean(camera_centers, axis=0)
        
        sky_distance = (scene_scale+ 10.0)  * 10.0
        sky_center = avg_camera_center + np.array([0, 0, sky_distance])
        
        sky_points = torch.from_numpy(sky_center).float() + scene_scale * 5.0 * (torch.rand((sky_num_pts, 3)) * 2 - 1)
        sky_rgbs = torch.zeros((sky_num_pts, 3))
        sky_rgbs[:, 2] = 0.8 + torch.rand(sky_num_pts) * 0.2
        sky_rgbs[:, 0] = 0.1 + torch.rand(sky_num_pts) * 0.2
        sky_rgbs[:, 1] = 0.3 + torch.rand(sky_num_pts) * 0.3
        
        points = torch.cat([points, sky_points], dim=0)
        rgbs = torch.cat([rgbs, sky_rgbs], dim=0)
        print(f"Sky gaussians added. Total points: {points.shape[0]}")
    ...
    params = [
        ...
        ("objects_dc", torch.nn.Parameter(objects_dc), 4e-3),
    ]
    ...
    # classifier = torch.nn.Linear(3, 2).cuda()
    classifier = torch.nn.Linear(3, 2).cuda()
...
class Runner:
    ...
    def __init__(self, cfg: Config) -> None:
        ...
        images_order = self.parser.image_names
        unordered_camtoworlds = self.parser.camtoworlds
        # assert len(self.parser.Ks_dict) == 1
        # K = self.parser.Ks_dict[1]
        for i in range(len(unordered_camtoworlds)):
            # get the index from images_order
            idx = images_order.index(f"{i:06d}.jpg")

            camtoworld = unordered_camtoworlds[idx]
            K = self.parser.Ks_dict[1]
        ...
        self.splats, splat_optimizers, self.classifier = create_splats_with_optimizers(
            ...
            device=self.device,
            sky_gaussians=cfg.sky_gaussians,
            sky_num_pts=cfg.sky_num_pts,
        )
        ...
        sky_mask_dir = os.path.join(self.cfg.work_dir, "stage1_reconstruction", "3d", "sky_masks")
        if os.path.exists(sky_mask_dir):
            # load the sky mask
            self.sky_mask = []
            for mask_path in sorted(os.listdir(sky_mask_dir)):
                mask = cv2.imread(os.path.join(sky_mask_dir, mask_path), cv2.IMREAD_GRAYSCALE)
                mask_tensor = torch.from_numpy(mask).float() / 255.0
                self.sky_mask.append(mask_tensor)
            self.sky_mask = torch.stack(self.sky_mask)
        else:
            self.sky_mask = None
    ...
    def rasterize_splats(
        ...
        **kwargs,
        ...
        objects_dc = self.splats["objects_dc"]  # [N, 4]
        objects_dc = objects_dc.view(-1, 3)  # [N, 4]
        objects_dc = self.classifier(objects_dc)
        ...
        if segmentation:
            classification_mask = (classification == 0)  # [N]
            final_foreground_mask = classification_mask.clone()  # [N]
   
            means = means[final_foreground_mask]
            quats = quats[final_foreground_mask]
            scales = scales[final_foreground_mask]
            opacities = opacities[final_foreground_mask]
            colors = colors[final_foreground_mask]
    ...
    def rasterize_splats_cls(
        ...
        **kwargs,
        ...
        quats = self.splats["quats"].clone().detach()  # [N, 4]
        scales = torch.exp(self.splats["scales"].clone().detach())  # [N, 3]
        opacities = torch.sigmoid(self.splats["opacities"].clone().detach())  # [N,]
        objects_dc = self.splats["objects_dc"]  # [N, 4]
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            #     masks = dilate_masks_lr(masks, kernel_size=3, iterations=1)
            ...
            if SEG_FLAG:
                ...
                colors_long = cls_colors.reshape(-1, 3).cuda()  # [N, 3]
                masks_long = masks.reshape(-1).long().cuda()           # [N]
                # get the masks to be flattened
                cls_output = self.classifier(colors_long)
                # softmax
                cls_output = torch.softmax(cls_output, dim=-1)
                cls_loss = F.cross_entropy(cls_output, masks_long)
                neighbor_loss = self.loss_cls_3d(
                    predictions=objects_dc_cls,
                    k=50,
                    lambda_val=4.0,
                    max_points=100000,
                    sample_size=5000
                )
                cls_loss = (cls_loss + neighbor_loss) * 0.1
            else:
                cls_loss = 0.0
            ...
            loss = l1loss * (1.0 - cfg.ssim_lambda) + ssimloss * cfg.ssim_lambda + cls_loss
            if cfg.depth_loss:
    ...
    def eval(self, step: int):
        ...
        for i, data in enumerate(valloader):
            ...
            if "mask" in data:
                ...
                render_part = canvas[:, w_half:, :]
                green_part = green[:, w_half:, :]
                mask_part = mask_bool
                render_part[mask_part] = (
                    (1 - alpha) * render_part[mask_part] + alpha * green_part[mask_part]
                )
                canvas[:, w_half:, :] = render_part
    ...
    def render_traj(self, step: int):
        ...
        for i in range(len(unordered_camtoworlds)):
            idx = images_order.index(f"{i:06d}.jpg")
            camtoworlds.append(unordered_camtoworlds[idx])
        ...
        for i in tqdm.trange(len(camtoworlds), desc="Rendering trajectory"):
            ...
            # write images
            canvas = torch.cat(
                [colors], dim=0 if width > height else 1
            )
        ...
        os.makedirs(video_dir, exist_ok=True)
        writer = imageio.get_writer(f"{video_dir}/traj_{step}.mp4", fps=12)
...
def update_config_from_yaml(cfg: Config, yaml_config: Dict) -> Config:
    ...
    cfg.work_dir = yaml_config["work_dir"]
    cfg.scene_name = yaml_config["scene_name"]
    
    if "sky_gaussians" in yaml_config:
        cfg.sky_gaussians = yaml_config["sky_gaussians"]
    if "sky_num_pts" in yaml_config:
        cfg.sky_num_pts = yaml_config["sky_num_pts"]
